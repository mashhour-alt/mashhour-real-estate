...
/**
 * Live translation subtitles — a self-contained floating widget.
 *
 * Drop it into any page (e.g. the map or the root layout) with <VoiceAssistant />.
 * It uses the browser's built-in speech recognition and a free translation API,
 * so there is no cost and no API key. Nothing is spoken back — the translation
 * appears as a single film-style caption near the bottom of the screen.
 */
...
export function VoiceAssistant({ arabic = false }: { arabic?: boolean }) {
  const ui = LABELS[arabic ? "ar" : "en"];
  ...
  return (
    <>
      ...
      {!supported && <div className="subtitle-error subtitle-error-static">{ui.notSupported}</div>}
    </>
  );
}
